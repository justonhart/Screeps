    ...
    if(this.carry.energy === this.carryCapacity){
        ...
        switch(this.room.operatingMode){
            ...
            case HARV_MODE:
                ...
                else if(this.room.find(FIND_RUINS, {filter: r => {return r.store[RESOURCE_ENERGY];}}).length){
                    switch(this.withdraw(this.room.find(FIND_RUINS, {filter: r => {return r.store[RESOURCE_ENERGY];}})[0], RESOURCE_ENERGY)){
                        case ERR_NOT_IN_RANGE:
                            this.moveTo(this.room.find(FIND_RUINS, {filter: r => {return r.store[RESOURCE_ENERGY];}})[0]);
                            break;
                        case 0:
                            this.memory.gathering = false;
                            break;
